...
function App() {
  ...
  const onShapePointerDown = (e, it) => {
    ...
    // Only initiate drag if this item is part of the new selection
    ...
    dragRef.current = {
      ...
      lockX: null,
      lockY: null,
      group: groupMembers, // always present (single = 1 member)
    };
    try { e.currentTarget.setPointerCapture(e.pointerId); } catch (_) {}
  };
  ...
  const onShapePointerMove = (e) => {
    ...
    setItems((prev) => {
      ...
      // Apply translation to all group members preserving their relative offsets
      return prev.map((it) => {
  ...
  }, [selectedIds, items]); // eslint-disable-line
...
function MultiInspector({ count, items, onColor, onDelete, onDuplicate, onClear }) {
  // Count by shape kind for a friendly summary
  const tally = {};
